...
public class Controller <ID, E extends Entity<ID>, ID2, E2 extends Entity<ID2>, ID3, E3 extends Entity<ID3>, ID4, E4 extends Entity<ID4>, ID5, E5 extends Entity<ID5>>{
    ...
    private EventService<ID5, E5> eventService;

    public Controller(UserService<ID, E> userService, FriendshipService<ID2, E2> friendshipService, MessageService<ID3, E3> messageService, RequestsService<ID4,E4> requestsService, EventService<ID5, E5> eventService) {
    ...
    public void addEvent(String description, List<User> participants, String date) {
        ID id = userService.getCurrentId();
        if( (Long) id == -1L)
            throw new LogInException();
        Event event = new Event(this.eventService.size() + 1, participants, description, date);
        this.eventService.add((E5) event);
    }

    public List<Event> getEventsNotParticipate(User user){
        Iterable<Event> events = (Iterable<Event>) this.eventService.getAll();
        List<Event> rez = new ArrayList<>();
        for(Event event:events){
            if(!event.getParticipants().contains(this.findOneByEmail(this.getCurrentEmail())))
                rez.add(event);
        }
        return rez;
    }

    public List<Event> getComingEvents(User user){
        Iterable<Event> events = (Iterable<Event>) this.eventService.getAll();
        List<Event> rez = new ArrayList<>();
        for(Event event:events) {
            if (event.getParticipants().contains(this.findOneByEmail(this.getCurrentEmail())) &&
                    LocalDate.parse(event.getDate(), Constants.DATE_TIME_FORMATTER_DP).getDayOfMonth() - LocalDateTime.now().getDayOfMonth() <= 4 &&
                    LocalDate.parse(event.getDate(), Constants.DATE_TIME_FORMATTER_DP).getDayOfMonth() - LocalDateTime.now().getDayOfMonth() >= 0 &&
                    LocalDate.parse(event.getDate(), Constants.DATE_TIME_FORMATTER_DP).getMonth() == LocalDateTime.now().getMonth() &&
                    LocalDate.parse(event.getDate(), Constants.DATE_TIME_FORMATTER_DP).getYear() == LocalDateTime.now().getYear())
                rez.add(event);
        }
        return rez;
    }
    ...
    public void updateEvent(Event event){
        this.eventService.update((E5) event);
    }
}
